  ...
  return (
    <Container className='flex flex-col gap-5 py-5' as='main'>
      <div className='felx flex-col gap-2'>
        <h1 className='text-2xl font-semibol capitalize'>{character.name}</h1>
      ...
      {character.skills && (
        <>
          ...
          <ul className='flexflex-wrap gap-1'>
        ...
        <>
          ...
          <ul className='grid gap-5'>
            {character_quotes.map((item, idx) => {
              return (
                <li
                  className='p-2 italix text-gray-400 border-1-4 border-green-400 rounded-md'
                  key={item.idx}
                >
                  {item.quote}
                </li>
